...
## 2026-06-18 - [UX] Optimize Mobile Data Entry with inputMode
**Learning:** Mobile browsers often present the standard alphanumeric keyboard for `<input type="number">` if `inputMode` is not explicitly set, adding friction to data entry by requiring users to manually switch to the numeric layout. Additionally, for whole numbers (like people or days), allowing decimals (`step="any"`) is both semantically incorrect and unhelpful.
**Action:** Always add an explicit `inputMode` to numeric inputs (`inputMode="decimal"` for floating-point values, `inputMode="numeric"` for integers). When accepting only integers, strictly enforce `step="1"` to guide the browser to provide the cleanest numeric keypad layout without a decimal separator, drastically reducing data entry friction on mobile devices.
